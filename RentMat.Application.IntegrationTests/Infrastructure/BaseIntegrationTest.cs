...
public class BaseIntegrationTest : IAsyncLifetime
{
    ...
    protected async Task<TokenResponseDto> RegisterUserAsync(string? login = null, string? email = null,
    ...
    protected async Task<User> CreateUserAsync(string? login = null, string? email = null,
        ...
        string password = "Password123", decimal balance = 0)
    {
        ...
        var user = new User
        {
            ...
            HashedPassword = BCrypt.Net.BCrypt.HashPassword(password, 4), Balance = balance
        };
    ...
    protected async Task<List<User>> CreateUsersAsync(int amount = 10)
    {
        ...
        var hashedPassword = BCrypt.Net.BCrypt.HashPassword(password, 4);
    ...
    protected async Task<Booking> CreateBookingAsync(DateTimeOffset? startDate = null, DateTimeOffset? endDate = null,
        decimal totalPrice = 0,
        BookingStatus? status = null, int? deviceId = null, int? userId = null, string? deviceName = null)
    {
        var user = await CreateUserAsync();
        var device = await CreateDeviceAsync(deviceName);
