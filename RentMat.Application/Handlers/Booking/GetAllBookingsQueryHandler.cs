...
public class GetAllBookingsQueryHandler : IRequestHandler<GetAllBookingsQuery, PagedResponse<BookingResponseDto>>
{
    ...
    public GetAllBookingsQueryHandler(AppDbContext db, IFusionCache cache, ILogger<GetAllBookingsQueryHandler> logger)
    {
        _db = db;
        _cache = cache;
        _logger = logger;
        ...
        CancellationToken cancellationToken)
    {
        ...
        var cacheKey =
            $"bookings:page:{query.Page}:page-size:{query.PageSize}:search:{search ?? string.Empty}:status:{status?.ToString() ?? "all"}";
